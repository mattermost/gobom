...
func generateComponents(path string) ([]*cyclonedx.Component, error) {
	...
	root.Name = path
	root.Version = unknownVersion
	root.PURL = gobom.PURL(gobom.GenericPackage, path, unknownVersion)
	root.root = true
...
func readLockfile(path string) (*lockfile, error) {
	data, err := ioutil.ReadFile(filepath.Join(path, podfileLock))
	if err != nil {
		return nil, err
	}
	lockfile := &lockfile{}
